...
const emptyPageUrl = () => urlencode(cache.getUrl(process.env.AWS_BUCKET_NAME, "emptypage.html"));
...
class AddedFileSpecDiff extends SpecDiff {
    getHtmlDiffUrl() {
        let head = urlencode(this.head.cache_url);
        return `https://services.w3.org/htmldiff?doc1=${ emptyPageUrl() }&doc2=${ head }`;
    }
}
...
class RemovedFileSpecDiff extends SpecDiff {
    getHtmlDiffUrl() {
        let base = urlencode(this.base.cache_url);
        return `https://services.w3.org/htmldiff?doc1=${ base }&doc2=${ emptyPageUrl() }`;
    }
}
