...
const express = require("express"),
    ...
    { logger, jobLogger, logStatus, memory } = require('./logger'),
...
// itself (Clever Cloud's monitoring agent polls it) are noise, and aren't
// logged.
const LOOPBACK = { "127.0.0.1": true, "::1": true, "::ffff:127.0.0.1": true };
function isNoise(req) {
    return req.path === "/health" || (req.socket && LOOPBACK[req.socket.remoteAddress] === true);
}
...
module.exports = function createApp(controller, config, log) {
    ...
    var app = express();
    // At debug level, one line when a request comes in with the process'
    // memory usage, so that a request that takes the process down can be
    // told apart from one that merely failed.
    app.use(function (req, res, next) {
        if (!isNoise(req) && log.isLevelEnabled("debug")) {
            log.debug({ method: req.method, url: req.originalUrl, memory: memory() }, "request received");
        }
        next();
    });
