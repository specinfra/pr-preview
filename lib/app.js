...
module.exports = function createApp(controller, config, logger) {
    ...
    // Everything else is acknowledged and logged as ignored, naming the event
    // and the PR or issue it concerned, so the log says what arrived.
    app.post('/github-hook', function (req, res, next) {
        if (config.nodeEnv != 'production' || (typeof req.isXHubValid === 'function' && req.isXHubValid())) {
            ...
            if (payload.issue_comment || payload.issue) {
                ...
                log(`Ignoring ${kind} "${action}" event on ${repo}#${payload.issue.number}: only pull_request events are handled`);
            } else if (payload.pull_request) {
                const id = `${payload.pull_request.base.repo.full_name}/${payload.number}`;
                if (payload.sender && payload.sender.login == "pr-preview[bot]") {
                    log(`Ignoring pull_request "${action}" event on ${id}: triggered by our own update`);
                } else if (!PR_ACTIONS.includes(action)) {
                    log(`Ignoring pull_request "${action}" event on ${id}: not an action we build on`);
                } else {
                    const queueResult = controller.queuePullRequest(payload);
                    if (!queueResult.queued) {
                        log(`Skipping pull_request "${action}" event on ${id}: ${queueResult.skipReason}`);
                    } else {
                        controller.processQueue(r => logResult(r, action));
                    }
                }
            } else {
                log(`Ignoring "${event}" event: only pull_request events are handled (payload keys: ${Object.keys(payload).join(", ")})`);
            }
        } else {
