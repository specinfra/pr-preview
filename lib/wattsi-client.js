...
// Wattsi requests wait in line, so a stalled one must eventually give up its turn.
const WATTSI_TIMEOUT = 10 * 60 * 1000;

// The Wattsi server doesn't cope with concurrent builds, so send it one request
// at a time across every PR this process is handling. Each call waits for the
// previous one to settle, whether it succeeded or failed.
let wattsiQueue = Promise.resolve();
function oneAtATime(fn) {
    const run = wattsiQueue.then(fn, fn);
    wattsiQueue = run.catch(() => {});
    return run;
}
...
class WattsiClient {
    ...
    fetch() {
        return mkdirpPromise(this.dirPath)
            .then(_ => oneAtATime(() => this.fetchZip(this.pr.head_sha, this.headZipPath)))
            .then(_ => oneAtATime(() => this.fetchZip(this.pr.merge_base_sha, this.mergeBaseZipPath)));
    }
