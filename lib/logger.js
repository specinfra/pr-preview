...
function createLogger(config) {
    ...
    return pino(options, pinoPretty({
        ...
        customPrettifiers: {
            err: prettifyError,
            reportingError: prettifyError,
            // A memory snapshot fits on one line, where it's easier to scan.
            memory: m => JSON.stringify(m)
        }
    }));
}
...
function installProcessHandlers(log, shutdown) {
    process.on("uncaughtException", (err, origin) => {
        log.fatal({ err, origin, memory: memory() }, "uncaught exception, exiting");
        process.exit(1);
    });
    process.on("unhandledRejection", reason => {
        const err = reason instanceof Error ? reason : new Error(`Unhandled rejection: ${String(reason)}`);
        log.fatal({ err, memory: memory() }, "unhandled promise rejection, exiting");
        process.exit(1);
    });
    process.on("warning", warning => {
        log.warn({ err: warning }, "process warning");
    });
    ["SIGTERM", "SIGINT", "SIGHUP"].forEach(signal => {
        process.on(signal, () => {
            log.info({ signal, memory: memory() }, `received ${signal}, shutting down`);
